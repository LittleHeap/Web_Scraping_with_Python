...
class WswpDb:
    ...
    def create_wswp_db(self, tb_name=None, tb_fields=None):
        ...
        if tb_name and tb_fields:
            try:
                # 连接数据库
                conn = psycopg2.connect(database=self.db_name, user=self.db_user, password=self.db_pass,
                                        host=self.db_ip, port=self.port)
                # 建立游标
                cursor = conn.cursor()
            except Exception, e:
                print
    ...
    def insert_wswp_db(self, table, fields, values):
        ...
        try:
            # 连接数据库
            conn = psycopg2.connect(database=self.db_name, user=self.db_user, password=self.db_pass, host=self.db_ip,
                                    port=self.port)
            # 建立游标
            cursor = conn.cursor()
        except Exception, e:
